...
import { Fetch_generar_contrato, Fetch_prorroga_contrato } from '@renderer/actions/actionsContratos';
...
function DataUser({ userContrato, userCargo, activeNewEmpleado, activeEdition, empleadores, activeSubModal }) {
  ...
  const GenerarContrato = async () => {
    dispatch(Fetch_generar_contrato(activeNewEmpleado.user, userCargo, contFilter, contFilterEmpleador, userData, setLoader, userId))
  };
    ...
    <>
      <div className={activeSubModal && activeNewEmpleado.subMenuEmpleado ? 'App__dashboard__contPageOutlet__PageUsers__menuUser__contDataUser__active' :
        ...
        <div className='App__dashboard__contPageOutlet__PageUsers__menuUser__contDataUser__contInfo'>
          ...
          <span>Empleador: <b>{contFilter.nombre_empleador}</b></span>
          ...
          <span>Estado: <b>{contFilter.estado} ({contFilter.estado_contrato})</b></span>
          <span>Salario: <b>{formatearNumero(String(contFilter.salario))}</b></span>
          {
            contFilter?.tipo_contrato == 'Fijo' || contFilter?.tipo_contrato == 'Fijo Manejo y Confianza' ? <>
          ...
          <div className='App__dashboard__contPageOutlet__PageUsers__menuUser__contDataUser__contInfo__contBtn'>
            {
              contFilter?.tipo_contrato == 'Fijo' || contFilter?.tipo_contrato == 'Fijo Manejo y Confianza' ? <button className='btn_style' type='submit' onClick={() =>
                dispatch(Fetch_prorroga_contrato(userData, contFilter))}>Prórroga</button> : null
            }
